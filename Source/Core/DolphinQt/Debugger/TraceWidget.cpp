...
void TraceWidget::AutoStep(CodeTrace::AutoStop option, const std::string reg)
{
  ...
  do
  {
    ...
    const AutoStepResults results = code_trace.AutoStepping(log_output, repeat, option);
...
void TraceWidget::OnRecordTrace(bool checked)
{
  ...
  bool timed_out = codetrace.RecordTrace(&m_code_trace, m_record_limit, 10, end_bp,
